...
const std::string YiyaGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString YiyaGUI::DEFAULT_WALLET = "~Default";

YiyaGUI::YiyaGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void YiyaGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Yiya address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and yiya: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to YIYA addresses"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Yiya addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Yiya addresses"));
    ...
    openAction->setStatusTip(tr("Open a yiya: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Yiya command-line options").arg(tr(PACKAGE_NAME)));
...
void YiyaGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/yiyacointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* yiyaLabelLayout = new QVBoxLayout(toolbarWidget);
        yiyaLabelLayout->addWidget(label);
        yiyaLabelLayout->addWidget(toolbar);
        yiyaLabelLayout->setDirection(QBoxLayout::TopToBottom);
        yiyaLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Yiya Coin Market Price"));
...
void YiyaGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool YiyaGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool YiyaGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void YiyaGUI::setWalletActionsEnabled(bool enabled)
{
...
void YiyaGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void YiyaGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void YiyaGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void YiyaGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void YiyaGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void YiyaGUI::openClicked()
{
...
void YiyaGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void YiyaGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void YiyaGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void YiyaGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void YiyaGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void YiyaGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** YIYA START */
void YiyaGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void YiyaGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void YiyaGUI::gotoManageAssetsPage()
{
...
void YiyaGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Yiya network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void YiyaGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void YiyaGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void YiyaGUI::updateHeadersSyncProgressLabel()
{
...
void YiyaGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void YiyaGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Yiya"); // default title
...
void YiyaGUI::changeEvent(QEvent *e)
{
...
void YiyaGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void YiyaGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void YiyaGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "YIYA")
        msg += tr("Amount: %1\n").arg(YiyaUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(YiyaUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void YiyaGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void YiyaGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool YiyaGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool YiyaGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void YiyaGUI::setHDStatus(int hdEnabled)
{
...
void YiyaGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void YiyaGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void YiyaGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void YiyaGUI::detectShutdown()
{
...
void YiyaGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void YiyaGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void YiyaGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(YiyaGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void YiyaGUI::subscribeToCoreSignals()
{
...
void YiyaGUI::unsubscribeFromCoreSignals()
{
...
void YiyaGUI::toggleNetworkActive()
{
    if (clientModel) {
    ...
    menu(0)
{
    ...
    QList<YiyaUnits::Unit> units = YiyaUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const YiyaUnits::Unit unit : units)
    {
        max_width = qMax(max_width, fm.width(YiyaUnits::name(unit)));
    }
...
{
    menu = new QMenu(this);
    for (YiyaUnits::Unit u : YiyaUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(YiyaUnits::name(u)), this);
...
{
    setText(YiyaUnits::name(newUnits));
}
...
void YiyaGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=YIYABTC"));
