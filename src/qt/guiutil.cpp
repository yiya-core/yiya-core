...
#include "yiyaaddressvalidator.h"
#include "yiyaunits.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Yiya address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
#endif
    widget->setValidator(new YiyaAddressEntryValidator(parent));
    widget->setCheckValidator(new YiyaAddressCheckValidator(parent));
}
...
bool parseYiyaURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no yiya: URI
    if(!uri.isValid() || uri.scheme() != QString("yiya"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!YiyaUnits::parse(YiyaUnits::YIYA, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseYiyaURI(QString uri, SendCoinsRecipient *out)
{
    // Convert yiya:// to yiya:
    //
    //    Cannot handle this later, because yiya:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("yiya://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "yiya:");
    }
    QUrl uriInstance(uri);
    return parseYiyaURI(uriInstance, out);
}

QString formatYiyaURI(const SendCoinsRecipient &info)
{
    QString ret = QString("yiya:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(YiyaUnits::format(YiyaUnits::YIYA, info.amount, false, YiyaUnits::separatorNever));
        paramCount++;
    }
...
bool openYiyaConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(YIYA_CONF_FILENAME);
    ...
    /* Open yiya.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Yiya.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Yiya (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Yiya (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "yiya.desktop";
    return GetAutostartDir() / strprintf("yiya-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a yiya.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Yiya\n";
        else
            optionFile << strprintf("Name=Yiya (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the yiya app
...
bool GetStartOnSystemStartup()
{
    CFURLRef yiyaAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (yiyaAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, yiyaAppUrl);

    CFRelease(yiyaAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add yiya app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, yiyaAppUrl, nullptr, nullptr);
    }
