...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** YIYA START */
    case OP_YIYA_ASSET              : return "OP_YIYA_ASSET";
    /** YIYA END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_YIYA_ASSET) { // OP_YIYA_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == YIYA_R) { // Check to see if YIYA starts at 27 ( this->size() < 105)
                if ((*this)[28] == YIYA_V)
                    if ((*this)[29] == YIYA_N)
                        index = 30;
            } else {
                if ((*this)[28] == YIYA_R) // Check to see if YIYA starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == YIYA_V)
                        if ((*this)[30] == YIYA_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == YIYA_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == YIYA_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == YIYA_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == YIYA_R) {
