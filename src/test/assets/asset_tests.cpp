...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of YIYACOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("YIYA", type));
        BOOST_CHECK(!IsAssetNameValid("YIYA", type));
        BOOST_CHECK(!IsAssetNameValid("YIYACOIN", type));

        //- Versions of YIYACOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("YIYA.COIN", type));
        BOOST_CHECK(IsAssetNameValid("YIYA_COIN", type));
        BOOST_CHECK(IsAssetNameValid("YIYASPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERYIYA", type));
        BOOST_CHECK(IsAssetNameValid("YIYASPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEYIYA", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_YIYAS", type));
        BOOST_CHECK(IsAssetNameValid("SEYIYAOT", type));
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("YIYA", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(Params().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
