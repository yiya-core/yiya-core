...
The RPM spec file provided here is for Yiya-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://yiyacore.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Yiya git repository](https://github.com/yiyacoin/core) but are not part of
...
Currently only `Source10` is used, for the example `yiya.conf` file.
...
in the yiya git repository. At present only an SVG version of the Yiya
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the yiya
...
needed when 0.12.1 is released, a proper fix is already in the Yiya git
master branch.
...
idea to build Yiya Core against a static version of that library compiled
...
The `yiya.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `yiya-server` package. At this point, most RPM based
...
The `yiya-server` package should have SELinux support. How to properly do
...
Yiya-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/yiya/). He has been
packaging Yiya for Fedora at least since 2012.
...
4. I renamed the `yiya` package that contains the Qt GUI to `yiya-core` as
that appears to be how the general population refers to it, in contrast to
`yiya-xt` or `yiya-classic`. I wanted to make sure the general population
